/* global themerealisebgcolor, themerealisecolor, jQuery, wp, _ */
/**
 ...
 */
 ...
 ( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( themerealisebgcolor ).forEach( function( context ) {
					var backgroundColorValue;
					if ( themerealisebgcolor[ context ].color ) {
						backgroundColorValue = themerealisebgcolor[ context ].color;
					} else {
						backgroundColorValue = wp.customize( themerealisebgcolor[ context ].setting ).get();
					}
					themerealisesetaccessiblecolorsvalue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( themerealisebgcolor ).forEach( function( context ) {
			wp.customize( themerealisebgcolor[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					themerealisesetaccessiblecolorsvalue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );
	 ...
	 * @return {void}
	 */
	function themerealisesetaccessiblecolorsvalue( context, backgroundColor, accentHue ) {
		var value, colors;
